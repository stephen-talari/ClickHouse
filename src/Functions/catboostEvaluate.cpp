...
{
private:
    using CatBoostLibraryBridgeHelperPtr = std::unique_ptr<CatBoostLibraryBridgeHelper>;
    mutable CatBoostLibraryBridgeHelperPtr bridge_helper;
...
public:
    ...
    void initBridge() const
    {
        /// TODO allow multiple models like the old loader could
        std::string library_path = getContext()->getConfigRef().getString("catboost_library_path");
        if (!std::filesystem::exists(library_path))
            throw Exception(ErrorCodes::FILE_DOESNT_EXIST, "Can't load library {}: file doesn't exist", library_path);

        std::string model_path = getContext()->getConfigRef().getString("catboost_model_path");
    ...
    DataTypePtr getReturnTypeFromLibCatboost() const
    {
    ...
    DataTypePtr getReturnTypeImpl(const ColumnsWithTypeAndName & arguments) const override
    {
        ...
        auto type = getReturnTypeFromLibCatboost();
    ...
    ColumnPtr executeImpl(const ColumnsWithTypeAndName & arguments, const DataTypePtr &, size_t) const override
    {
        ...
        ColumnsWithTypeAndName data_arguments(arguments.begin() + 1, arguments.end());
        for (auto & arg : data_arguments)
        {
        ...
        auto res = bridge_helper->evaluate(data_arguments);
